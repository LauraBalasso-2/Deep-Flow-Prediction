...
class UNet(nn.Module):
    def __init__(self, latent_size, channelExponent=6, dropout=0.):
        ...
        # Progressively reduce spatial size to 1x1
        ...
        self.layer6 = blockUNet(channels * 8, channels * 8, 'layer6', transposed=False, bn=False, relu=False,
                                dropout=dropout, size=2)

        # Bottleneck removed, and we start decoding from the provided matrix (fixed latent input)
        self.dlayer_bottleneck = nn.ConvTranspose2d(latent_size, channels * 8, kernel_size=2, stride=1, padding=0)
    ...
    def forward(self, x, latent_code):
        ...
        # Instead of the bottleneck, we use the fixed_matrix
        # Ensure fixed_matrix has shape [batch_size, latent_size, 1, 1]
